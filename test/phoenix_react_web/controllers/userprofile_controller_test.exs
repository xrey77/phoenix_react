defmodule PhoenixReactWeb.UserprofileControllerTest do
  ...
  describe "index" do
    test "lists all userprofiles", %{conn: conn} do
      conn = get(conn, ~p"/api/userprofiles")
  ...
  describe "create userprofile" do
    test "renders userprofile when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/api/userprofiles", userprofile: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, ~p"/api/userprofiles/#{id}")
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/api/userprofiles", userprofile: @invalid_attrs)
  ...
  describe "update userprofile" do
    ...
    test "renders userprofile when data is valid", %{conn: conn, userprofile: %Userprofile{id: id} = userprofile} do
      conn = put(conn, ~p"/api/userprofiles/#{userprofile}", userprofile: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, userprofile: userprofile} do
      conn = put(conn, ~p"/api/userprofiles/#{userprofile}", userprofile: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete userprofile" do
    ...
    test "deletes chosen userprofile", %{conn: conn, userprofile: userprofile} do
      conn = delete(conn, ~p"/api/userprofiles/#{userprofile}")
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, ~p"/api/userprofiles/#{userprofile}")
      end
    end
  end
